...
{
	/// <summary>
	/// Summary description for Cmd_AppendToFile.
	/// </summary>
	internal class Cmd_AppendToFile : AsyncBase, IDisposable
	{
		#region AsyncResult class
		class AppendToFile_SO : AsyncResultBase
		{
			Stream _userStream = null;

			internal AppendToFile_SO(Stream userStream, 
				AsyncCallback cb, 
		...
		internal Cmd_AppendToFile(FtpClient ftp)
		{
			_client = ftp;
		}
		...
		{
			get { return typeof(AppendToFile_SO); }
		}
		...
		void CreateDTP()
		{
			...
			if(_disposed)
				throw new ObjectDisposedException("Cmd_AppendToFile");
		}
		...
		internal IAsyncResult BeginExecute(int timeout, 
			...
			object state)
		{
			AppendToFile_SO stateObj = null;
			SetProgress(true);
			try
			{
				CreateDTP();
				stateObj = new AppendToFile_SO(userStream,
					cb,
					state);
		...
		{
			AppendToFile_SO stateObj = (AppendToFile_SO)ar.AsyncState;
			try
			{
		...
		{
			AsyncBase.VerifyAsyncResult(ar, typeof(AppendToFile_SO));
