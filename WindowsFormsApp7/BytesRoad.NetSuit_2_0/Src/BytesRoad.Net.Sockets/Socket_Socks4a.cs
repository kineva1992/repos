...
namespace BytesRoad.Net.Sockets
{
	...
	internal class Socket_Socks4a : SocketBase
	{
		...
		{
			Socket_Socks4a _baseSocket;
			int _readBytes = 0;
			IPAddress _proxyIP = null;

			internal Bind_SO(Socket_Socks4a baseSocket,
				AsyncCallback cb, 
				object state) : base(cb, state)
			{
		...
		null)
	{
		...
		void VerifyResponse()
		{
			...
			if(_response[1] != 90)
			{
				...
				if(91 == err)
					...
					msg = string.Format("Socks4a: Request rejected because SOCKS server cannot connect to identd on the client ({0}).", err);
				else if(93 == err)
					msg = string.Format("Socks4a: Request rejected because the client program and identd report different user-ids ({0}).", err);
				else
		...
		byte[] PrepareCmd(string remoteHost, int remotePort, byte cmdVal)
		{
			...
			byte[] cmd = new byte[8+userLength+1+hostLength+1];
			cmd[0] = 4;
			cmd[1] = cmdVal;
			...
			Array.Copy(Encoding.Default.GetBytes(remoteHost),
				0,
				cmd, 
				8+userLength+1, 
				hostLength);
			...
			object state)
		{
			...
			try
			{
				...
				//
				BeginGetHostByName(_proxyServer,
					new AsyncCallback(this.Bind_GetHost_End),
					stateObj);
			}
			catch(Exception ex)
			{
